...
function solution(dartResult) {
    ...
    for (const point of dartResult) {
        if (point >= 0 && point <= 9 ) { 
            total = total + point;
        }
        else if (point == 'S'){ 
            answer.push(Math.pow(total, 1));
            total = '';
        }
        else if (point == 'D'){ 
            answer.push(Math.pow(total, 2));
            total = '';
        }
        else if (point == 'T'){ 
            answer.push(Math.pow(total, 3));
            total = '';
        }
        else if (point == '*'){ // (직전 점수 + 해당 점수) * 2
            if (answer.length > 1) {
